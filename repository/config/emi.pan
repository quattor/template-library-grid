...
variable EMI_REPOSITORY_LIST ?= {
  SELF[length(SELF)] = REPOSITORY_EMI_PREFIX + '_base';
  SELF[length(SELF)] = REPOSITORY_EMI_PREFIX + '_third_party';
  SELF[length(SELF)] = REPOSITORY_EMI_PREFIX + '_updates';
  SELF[length(SELF)] = 'wlcg_x86_64';
  SELF[length(SELF)] = 'mpi';
  if ( !is_defined(YUM_SNAPSHOT_DATE) ) {
    SELF[length(SELF)] = REPOSITORY_EMI_PREFIX + '_external';
    SELF[length(SELF)] = REPOSITORY_EMI_PREFIX + '_unofficial';
    SELF[length(SELF)] = REPOSITORY_EMI_PREFIX + '_test';
    SELF[length(SELF)] = REPOSITORY_EMI_PREFIX + '_sam';
