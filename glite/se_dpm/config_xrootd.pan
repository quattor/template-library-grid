...
unique template glite/se_dpm/config_xrootd;
...
variable XROOTD_AUTH_USE_TOKEN ?= if ( is_defined(XROOTD_TOKEN_AUTH_PARAMS) ) {
                                    true;
                                  } else {
                                    false;
                                  };
...
variable DPM_XROOTD_SHARED_KEY ?= if ( XROOTD_AUTH_USE_TOKEN ) {
                                undef;
                              } else {
...
variable XROOTD_TOKEN_AUTH_PARAMS = {
  if ( XROOTD_AUTH_USE_TOKEN ) {
    # exportedVOs can be specified either as a list or nlist. Convert to a nlist for easier further processing
    # if this is a list.
...
'/software/components/filecopy/services' = {
  SELF[escape(DPM_XROOTD_SYSCONFIG_FILE+'.templ-quattor')] = nlist('source', DPM_XROOTD_SYSCONFIG_FILE+'.dpm-templ',
                                                                   'owner', 'root:root',
                                                                   'perms', '0644',
  ...
  if ( is_defined(DPM_XROOTD_SHARED_KEY) ) {
    SELF[escape(DPM_XROOTD_SHARED_KEY_FILE)] =  nlist('config', DPM_XROOTD_SHARED_KEY,
                                                      'owner', 'dpmmgr:dpmmgr',
                                                      'perms', '0640'
                                                     );
  };
  ...
  if( is_defined(XROOTD_MODIFIED_STARTUP_FUNCTIONS) && XROOTD_MODIFIED_STARTUP_FUNCTIONS ) {
    SELF[escape('/etc/rc.d/init.d/xrootd-functions')] = nlist(
                'config',file_contents('glite/se_dpm/templ/xrootd-functions.templ'),
                'owner', 'root:root',
                'perms', '0755',);
  };
