# This template contains the proxy renewal script template as provided
# by RPM lcg-vobox. The script template is broken out in 3 parts in this
# template:
#  - Initial part; until the variables defining VO name and the enabled
#    user, used as is.
#  - Variables: will be replaced by the actual VO name and user.
#  - Final part: the end of the script, used as is.

structure template glite/vobox/proxy-renewal-script;
...
. /etc/profile.d/grid-env.sh || {
    do_failure "$0: cannot set up VOBOX environment"
    exit 1
}

#C. DIARRA : 13/07/2012 : source de grid-proxy-init sinon grid-proxy-init not found
