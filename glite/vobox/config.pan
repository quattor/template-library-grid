unique template glite/vobox/config;
...
# Normally there should be only one enabled VO, this has been checked before (glite/vobox/init)
...
'/software/components' = {
  ...
  if ( is_defined(VOBOX_ENABLED_VOS_USER) ) {
    foreach (vo;user;VOBOX_ENABLED_VOS_USER) {
      script = create('glite/vobox/proxy-renewal-script');
      contents = script['initial'];
      variables = replace('(?m:THEVO)',vo,script['variables']);
