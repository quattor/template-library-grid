...
variable GLITE_ARCH_LIB ?= '/lib64';

variable DEFAULT_DPM_HOST ?= {
    if ( is_defined(DPM_HOST) ) {
        DPM_HOST;
    } else {
        undef;
    };
...
# Path-like variables.
'/software/components/profile/path/PATH/prepend' = push(
    GLITE_LOCATION + '/bin',
    GLITE_LOCATION + '/externals/bin'
);

# For LD_LIBRARY_PATH on 64-bit both lib64 and lib are needed for 32-bit
# apps to work
'/software/components/profile/path/LD_LIBRARY_PATH/prepend' = push(
    GLITE_LOCATION + GLITE_ARCH_LIB + '/classads',
    GLITE_LOCATION + GLITE_ARCH_LIB,
    GLITE_LOCATION + '/lib'
);
'/software/components/profile/path/PYTHONPATH/prepend' = push(
    GLITE_LOCATION + GLITE_ARCH_LIB + '/python'
);
'/software/components/profile/path/PERLLIB/prepend' = push(
    GLITE_LOCATION + GLITE_ARCH_LIB + '/perl'
);
'/software/components/profile/path/MANPATH/append' = push(
    GLITE_LOCATION + '/share/man'
);
