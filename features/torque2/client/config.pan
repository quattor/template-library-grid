...
"/software/components/pbsclient/directPaths" = {
  if ( is_defined(WN_SHARED_AREAS) ) {
    if ( !is_dict(WN_SHARED_AREAS) ) {
      error("WN_SHARED_AREAS must be a dict");
    };
  ...
  if ( (length(SELF) == 0) && is_defined(CE_SHARED_HOME) && CE_SHARED_HOME ) {
    ...
    SELF[length(SELF)] = dict("locations",'*.'+DOMAIN+':/home',
                              "path","/home"
                             );
  };
  ...
  if ( is_defined(CREAM_SANDBOX_MPOINTS) ) {
    foreach (ce;mpoint;CREAM_SANDBOX_MPOINTS) {
      ...
      if ( exists(sandbox_path) ) {
        ...
        SELF[length(SELF)] = dict("locations",ce+':'+sandbox_path,
                                  "path",mpoint,
                                 );
      } else {
...
"/software/components/cron/entries" = if ( is_defined(PBS_MONITORING_SCRIPT) ) {
                                        push(dict("name","pbs-monitoring",
                                                  "user","root",
                                                  "timing", dict("minute", PBS_MONITORING_INTERVAL_MINUTE),
...
"/software/components/altlogrotate/entries/mom-logs" =
  dict("pattern", "/var/log/mom-logs.ncm-cron.log",
       "compress", true,
       "missingok", true,
       "frequency", "weekly",
       "create", true,
