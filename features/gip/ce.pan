...
'{lcg-info-dynamic-condor}' ?= if(CE_BATCH_SYS == 'condor'){nlist()}else{null};
...
"/software/components/gip2/plugin" = {
  ...
  if ( is_defined(CE_BATCH_SYS_LIST) && (length(CE_BATCH_SYS_LIST) > 0) ) {
    ...
    foreach (jobmanager;lrms;CE_BATCH_SYS_LIST) {
      if ( lrms == "condor" ) {
        contents = contents + LCG_INFO_SCRIPTS_DIR +"/lcg-info-dynamic-condor /usr/bin/ "+
...
'/software/components/symlink/links' = {
    ...
    #This is more like a Workaround. We need a more proper solution.
    if (CE_BATCH_SYS == 'condor' ){
        append(nlist(
            "name", '/usr/libexec/lcg-info-dynamic-condor',
            "target", '/opt/glite/libexec/lcg-info-dynamic-condor',
            "replace", nlist("all", "yes"),
        ));       
    };
