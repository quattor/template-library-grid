...
function add_vo_infos = {
    ...
    if ( is_defined(site_vo_params['voms_servers']) ) {
      ...
      foreach (i;site_server;site_vo_params['voms_servers']) {
        ...
        if ( is_defined(site_server['host']) ) {
          ...
          error("VO "+vo+": site parameters contain an invalid VOMS server entry ('host' and 'name' both undefined)");
        };
        found = false;
        ok = first(voms_servers,i,std_server);
        while ( ok && !found ) {
          if ( !is_defined(std_server[voms_server_selector]) ) {
            error("VO "+vo+": '"+voms_server_selector+"' parameter missing for VOMS server entry "+to_string(std_server));
    ...
    if ( is_defined(site_vo_params['voms_mappings']) ) {
      ...
      foreach (i;site_mapping;site_vo_params['voms_mappings']) {
        ...
        if ( is_defined(site_mapping['fqan']) ) {
          ...
          error("VO "+vo+": site parameters contain an invalid VOMS mapping definition ('fqan' and 'description' both undefined)");
        };
        found = false;
        ok = first(voms_mappings_tmp,i,std_mapping);
        while ( ok && !found ) {
          if ( !is_defined(std_mapping[voms_mapping_selector]) ) {
            error("VO "+vo+": '"+voms_mapping_selector+"' parameter missing for VOMS mapping entry "+to_string(std_mapping));
  ...
  foreach (i;mapping;voms_mappings_tmp) {
    ...
    if ( mapping['enabled'] ) {
      if ( !is_defined(mapping['description']) || !is_defined(mapping['suffix']) ) {
        error(function_name+': missing mapping suffix or description in mapping number '+to_string(j));
      };
      if ( !is_defined(mapping['fqan']) ) {
        error(function_name+': missing VOMS FQAN for VO '+vo_name+' role '+mapping['description']);
      } else if ( !is_string(mapping['fqan']) ) {
        error(function_name+": 'fqan' attribute is not a string for VO "+vo_name+" group/role "+mapping['description']);
      };      
      if ( !VO_USE_LEGACY_ACCOUNT_SUFFIX ) {
        if ( is_defined(mapping['suffix2']) ) {
          mapping['suffix'] = mapping['suffix2'];
        } else {
          error("VO "+vo+": new account suffix enabled but 'suffix2' attribute not present for FQAN "+to_string(mapping['fqan']));
        };      
      };
      ...
      if ( is_defined(account_mapping_number[mapping['suffix']]) ) {
        ...
        debug("FQAN "+mapping['fqan']+": merge with mapping number "+to_string(previous_mapping)+
                                                                 " which uses the same account suffix ("+mapping['suffix']+")");
  ...
  if (match(FULL_HOSTNAME,vo_params[vo]['unlock_accounts'])) {
    ...
    if ( (is_defined(VOBOX_ENABLED_VOS) && (index(vo,VOBOX_ENABLED_VOS) > 0)) ||
      ...
      error('VO '+vo+' unlock_account ('+vo_params[vo]['unlock_accounts']+') conflicts with gsissh requirements. Check VOS_SITE_PARAMS['+
                                      vo+']["unlock_accounts"] and VOS_SITE_PARAMS["DEFAULT"].');
    };
    # Locked account
    vo_account_pass = '!*NP*';
  };
  ...
  #  
  # Configure  VO-specific services (MyProxy, EDG RB, gLite WMS...)
  # For RB/WMS one host can be used for all services or a different host for each service.
  ...
  if ( is_defined(vo_params[vo]['rb_hosts']) ) {
    if ( is_list(vo_params[vo]['rb_hosts']) ) {
      foreach (i;host;vo_params[vo]['rb_hosts']) {
        result['services']['edg']['lbhosts'] = list(host);
        result['services']['edg']['nshosts'] = list(host);
      };
    } else {
      host = vo_params[vo]['rb_hosts'];
      result['services']['edg']['lbhosts'] = list(host);
      result['services']['edg']['nshosts'] = list(host);
    };
  };
  
  if ( is_defined(vo_params[vo]['nshosts']) ) {
    if ( is_list(vo_params[vo]['nshosts']) ) {
      result['services']['edg']['nshosts'] = vo_params[vo]['nshosts'];
    } else {
      result['services']['edg']['nshosts'] = list(vo_params[vo]['nshosts']);
    };
  };
  
  if ( is_defined(vo_params[vo]['lbhosts']) ) {
    if ( is_list(vo_params[vo]['lbhosts']) ) {
      result['services']['edg']['lbhosts'] = vo_params[vo]['lbhosts'];
    } else {
      result['services']['edg']['lbhosts'] = list(vo_params[vo]['lbhosts']);
    };
  };


  if ( is_defined(vo_params[vo]['wms_hosts']) ) {
    if ( is_list(vo_params[vo]['wms_hosts']) ) {
      result['services']['glite']['lbhosts'] = vo_params[vo]['wms_hosts'];
      result['services']['glite']['nshosts'] = vo_params[vo]['wms_hosts'];
      tmp_list=list();
      foreach (i;host;vo_params[vo]['wms_hosts']) {
        tmp_list=merge(tmp_list,list('https://'+host+':7443/glite_wms_wmproxy_server'));
      };      
      result['services']['glite']['wmproxies'] = tmp_list;
    } else {
      host = vo_params[vo]['wms_hosts'];
      result['services']['glite']['lbhosts'] = list(host);
      result['services']['glite']['nshosts'] = list(host);
      result['services']['glite']['wmproxies'] = list('https://'+host+':7443/glite_wms_wmproxy_server');
    };
  };
  
  if ( is_defined(vo_params[vo]['wms_nshosts']) ) {
    if ( is_list(vo_params[vo]['wms_nshosts']) ) {
      result['services']['glite']['nshosts'] = vo_params[vo]['wms_nshosts'];
    } else {
      result['services']['glite']['nshosts'] = list(vo_params[vo]['wms_nshosts']);
    };
  };
  
  if ( is_defined(vo_params[vo]['wms_lbhosts']) ) {
    if ( is_list(vo_params[vo]['wms_lbhosts']) ) {
      result['services']['glite']['lbhosts'] = vo_params[vo]['wms_lbhosts'];
    } else {
      result['services']['glite']['lbhosts'] = list(vo_params[vo]['wms_lbhosts']);
    };
  };

  if ( is_defined(vo_params[vo]['wms_proxies']) ) {
    if ( is_list(vo_params[vo]['wms_proxies']) ) {
      result['services']['glite']['wmproxies'] = vo_params[vo]['wms_proxies'];
    } else {
      result['services']['glite']['wmproxies'] = list(vo_params[vo]['wms_proxies']);
    };
  };
  ...
  if ( length(voms_servers) > 0 ) {
    ...
    if ( is_defined(VO_VOMS_FQAN_FILTER) ) {
      ...
      if ( is_string(fqan_filter_tmp) ) {
        ...
        error('VO_VOMS_FQAN_FILTER entry for VO '+vo_name+' has an invalid format: must be a string or list');
      };
          ...
          if ( !match(filter_value,'/') && is_defined(voms_mappings) ) {
            ...
            if ( !fqan_not_found ) {
              ...
              foreach (j;fqan;mapping_params['fqan']) {
                ...
                debug('FQAN filter value "'+filter_value+'" for VO '+vo_name+' replaced by "'+fqan_filter[length(fqan_filter)-1]+'"');
              };
            } else {
              debug('No FQAN found in VOMS mappings for VO '+vo_name+' matching description "'+filter_value+'"');
            };
          } else {
        ...
        if (  mapping['enabled'] ) {
          ...
          foreach (i;fqan;mapping['fqan']) {
            if ( is_defined(fqan_enabled[escape(fqan)]) ) {
              error('FQAN '+fqan+' is already a member of a voms_mapping entry other than '+mapping['description']);
            } else if ( is_defined(fqan_filter) && index(fqan,fqan_filter) == -1 ) {
              fqan_enabled[escape(fqan)] = false;
              debug('FQAN '+fqan+' does not match filter ('+to_string(fqan_filter)+'): marked as disabled');
            } else {
    ...
    foreach (j;mapping;voms_mappings) {
      ...
      if ( !is_defined(mapping['pool_size']) ) {
        if ( is_sw_mgr && is_defined(vo_params[vo]['swmgr_pool_accounts_disabled']) && vo_params[vo]['swmgr_pool_accounts_disabled'] ) {
          mapping['pool_size'] = 1;
        } else {
      ...
      if ( exists(mapping['is_prefix']) ) {
        home_attrs = vo_get_dir_attrs(vo,vo_params[vo],mapping['suffix'],is_sw_mgr,undef,mapping['is_prefix']);
      } else {
      ...
      # Group associated with VO is created later as part of creation of pool accounts for normal users.
        ...
        if ( is_defined(mapping['base_uid']) ) {
          ...
          debug('Base UID explicitly defined for mapping '+mapping['description']+': risk of conflict with other mappings, try to remove it.');
        } else {
      ...
      if ( (role_base_uid > pool_first_uid) && (role_base_uid < pool_last_uid) ) {
        error('Base UID for mapping '+mapping['description']+' conflicting with pool accounts for normal users');
      };
      ...
      if ( mapping['enabled'] ) {
        ...
        if ( !is_defined(result['accounts']['users'][mapped_user]) ) {      
          ...
          # Create pool accounts for the FQAN
            ...
            debug('Creating pool accounts '+mapped_user+' for mapping '+mapping['description']+': base UID='+to_string(role_base_uid)+
                                                                                           ', pool_size='+to_string(mapping['pool_size']));
            ...
            if ( is_defined(mapping['gid']) ) {
              ...
              debug('GID for mapping '+mapping['description']+' group explicitly defined: risk of conflict, try to remove.');
            } else {
      ...
      if (mapping['enabled']) {
        foreach (fqan_num;fqan;mapping['fqan']) {
          ...
          if ( fqan_enabled[escape(fqan)] ) {
            #debug('FQAN='+fqan+', vobox_enabled_user='+to_string(mapped_user)+', fqan_filter='+to_string(fqan_filter));
            if ( !is_defined(result['vobox_user']) &&
                 is_defined(VOBOX_ENABLED_VOS) && 
                 is_defined(fqan_filter) &&
                 (index(vo_name,VOBOX_ENABLED_VOS) >= 0) ) {
  ...
  if ( !is_defined(result['accounts']['users'][sw_mgr_user]) ) {
    debug('SW manager account not defined for VO '+vo_name+': creating a SW manager account ('+sw_mgr_user+') with default parameters');
  ...
  if ( exists(result['accounts']["users"][sw_mgr_user]["poolSize"]) ) {
    ...
    sw_mgr_user = sw_mgr_user+replace(" ",
                                      "0",
                                      format('%'+to_string(result['accounts']["users"][sw_mgr_user]["poolDigits"])+'d',
  ...
  if ( is_defined(result['swarea']['name']) ) {
    if ( exists(mapping['is_prefix']) ) {
      swarea_attrs = vo_get_dir_attrs(vo,vo_params[vo],undef,undef,result['swarea']['name'],mapping['is_prefix']);
    } else {
      swarea_attrs = vo_get_dir_attrs(vo,vo_params[vo],undef,undef,result['swarea']['name'],undef);
    };
...
function combine_dirperm_paths = {
  ...
  # Check cardinality.
  if ((ARGC < 1)  || (ARGC > 2) ) error("usage: '"+comp_base+"' = "+function_name+"(vos,path_type)");
  ...
  if ( length(SELF["paths"]) == 0 ) {
    ...
    if ( !exists(SELF['dependencies']['pre']) || 
         !is_list(SELF['dependencies']['pre']) ) {
...
function combine_vo_env = {
  ...
  # Check cardinality.
  if (ARGC < 3 ) error("usage: '"+comp_base+"' = "+function_name+"(vos,vo_areas,default_SE[,default_SE_SC3])");
...
function vo_get_dir_attrs = {
  ...
  if ( exists(ARGV[4]) && is_defined(ARGV[4]) && (length(ARGV[4]) > 0) ) {
    ...
    if ( !is_defined(result['directory']) && sw_mgr_home && exists(VO_SWMGR_HOMES['DEFAULT']) && is_defined(VO_SWMGR_HOMES['DEFAULT']) ) {
      if ( is_prefix ) {
    ...
    if ( !is_defined(result['directory']) &&
         exists(NODE_VO_DEFAULT_HOMEROOT) &&
...
function add_vos_to_yaim = {
    ...
    foreach ( i; vo; VO_CONFIG ) {
        ...
        foreach ( j; voms; vo['voms_servers'] ) {
            ...
            voms_servers = voms_servers
                         + "'vomss://" + voms['host'] + ":" + srv['port']
                         + "/voms/" + voname  + "?/" + voname + "/' ";
            vomses       = vomses
                         + "'" + voname + " " + voms['host'] + " " 
                         + voms['port'] + " " + srv['dn'] + " "
                         + voname + "' ";
            voms_ca_dn   = voms_ca_dn
                         + "'" + srv['CA-dn'] + "' ";
        };
