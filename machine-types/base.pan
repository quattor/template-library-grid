...
variable OS_NS_CONFIG_EMI = OS_NS_CONFIG + 'emi/' + EMI_VERSION + '/';
...
#
include { if ( NFS_SERVER_ENABLED && !OS_POSTPONE_NFS_CONFIG ) 'common/nfs/server/config' };
include { if ( NFS_CLIENT_ENABLED && !OS_POSTPONE_NFS_CONFIG ) 'common/nfs/client/config' };
...
variable GLITE_OS_POSTCONFIG ?= OS_NS_CONFIG_EMI+'postconfig';
