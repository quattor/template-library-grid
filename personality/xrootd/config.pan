...
unique template personality/xrootd/config;
...
'/software/components/filecopy/services' = {
  if( index('disk',XROOTD_SERVER_ROLES) >= 0 ) {
    ...
    if ( is_defined(XROOTD_DISK_CONFIG_NAME) ) {
      ...
      config_template_src = 'personality/xrootd/templates/disk_config';
  ...
  if( index('redir',XROOTD_SERVER_ROLES) >= 0 ) {
    ...
    if ( is_defined(XROOTD_REDIR_CONFIG_NAME) ) {
      ...
      config_template_src = 'personality/xrootd/templates/redir_config';
      src = create(config_template_src);
      SELF[escape(config_template_path)] = nlist('config', src['content'],
                                                 'owner', 'root:root',
                                                 'perms', '0644',
...
include { if ( XROOTD_FEDERATION_ENABLED ) 'personality/xrootd/config_federation' };

# Configure Xrootd monitoring.
# Must be done last.
include { 'personality/xrootd/monitoring' };
