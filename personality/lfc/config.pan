...
"/software/components/dpmlfc/options/lfc/db/server" ?= {
    if ( is_defined(LFC_DB_PARAMS['server']) ) {
...
"/software/components/dpmlfc" = {
    foreach (i; service; LFC_ENABLED_SERVICES) {
        ...
        SELF[service] = dict(FULL_HOSTNAME, host_params);
    };
    SELF;
};
...
'/software/components/mysql/databases' = {
    ...
    SELF[LFC_DB_NAME]['users'][LFC_DB_PARAMS['user']] = dict(
        'password', LFC_DB_PARAMS['password'],
...
variable LFC_IPTABLES_INCLUDE = {
    if ( is_defined(LFC_IPTABLES_RULES) && (length(LFC_IPTABLES_RULES) > 0) ) {
        'personality/lfc/iptables';
    } else {
        null;
    }
};
