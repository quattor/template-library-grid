...
variable CE_CLEANUP_ACCOUNTS_CONFIG = {
    ...
    foreach (vo; vo_params; VO_INFO) {
        foreach (user; params; vo_params['accounts']['users']) {
            if (
                (!exists(params['poolSize']) || !is_defined(params['poolSize']) || (params['poolSize'] == 0)) &&
...
"/software/components/filecopy/services" = {
    SELF[escape(CE_CLEANUP_ACCOUNTS_CONFIG_FILE)] = dict(
        'config', CE_CLEANUP_ACCOUNTS_CONFIG,
        'perms', '0755',
        'owner', 'root:root',
    );
    SELF;
    ...
    push(
        dict(
            ...
            "command", "PATH=/sbin:/bin:/usr/sbin:/usr/bin; " +
                "/usr/sbin/cleanup-grid-accounts.sh -v -c " +
                CE_CLEANUP_ACCOUNTS_CONFIG_FILE + ' ' +
                CE_CLEANUP_ACCOUNTS_BASE_OPTIONS + ' ' +
                CE_CLEANUP_ACCOUNTS_SITE_OPTIONS +
...
"/software/components/altlogrotate/entries/cleanup-grid-accounts" = {
    dict(
        "pattern", "/var/log/cleanup-grid-accounts.ncm-cron.log",
        "compress", true,
        "missingok", true,
        "frequency", "monthly",
        "create", true,
