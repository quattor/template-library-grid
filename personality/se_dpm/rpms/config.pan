...
'/software/packages' = {
    # Backward compatibility: keep metapackage in the config if
    # SEDPM_USE_METAPACKAGES=true. This is necessary to workaround a problem
    # with YUM that uninstall all the RPMs required by the metapackage when
    # uninstalling the metapackage. To switch from using the matapackage to not
    # using it, be sure to remove the metapackage manually BEFORE deploying the
    # configuration change to workaround the YUM problem.
