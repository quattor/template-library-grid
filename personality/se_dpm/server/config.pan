...
"/software/components/dpmlfc/options/dpm/db/server" ?= {
    if ( is_defined(DPM_DB_PARAMS['server']) ) {
        DPM_DB_PARAMS['server'];
    } else {
        null;
    };
};
"/software/components/dpmlfc/options/dpm/db/infoUser" ?= {
    if ( is_defined(DPM_DB_PARAMS['infoUser']) ) {
        DPM_DB_PARAMS['infoUser'];
    } else {
        null;
    };
};
"/software/components/dpmlfc/options/dpm/db/infoPwd" ?= {
    if ( is_defined(DPM_DB_PARAMS['infoPwd']) ) {
        DPM_DB_PARAMS['infoPwd'];
    } else {
        null;
    };
};
"/software/components/dpmlfc/options/dpm/db/infoFile" ?= {
    if ( is_defined(DPM_DB_PARAMS['infoFile']) ) {
...
include if (DPM_MYSQL_SERVER == FULL_HOSTNAME) 'personality/se_dpm/server/mysql';
...
'/software/components/filecopy/services' = {
    if ( is_defined(GIP_SCRIPT_DPM_DYNAMIC) ) {
        SELF[escape(GIP_SCRIPT_DPM_DYNAMIC_NAME)] = dict(
            'config', GIP_SCRIPT_DPM_DYNAMIC,
            'owner', 'root:root',
            'perms', '0755'
        );
    };
